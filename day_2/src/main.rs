...
Answer to Part 1 was 
Answer to Part 2 was 
...
fn possible(s: &String) -> bool {
    ...
    let red: Vec<u32> = red_regex.captures_iter(s).filter_map(|c| c.get(1).map(|m| m.as_str().parse::<u32>().ok())).flatten().collect();
    let blue: Vec<u32> = blue_regex.captures_iter(s).filter_map(|c| c.get(1).map(|m| m.as_str().parse::<u32>().ok())).flatten().collect();
    let green: Vec<u32> = green_regex.captures_iter(s).filter_map(|c| c.get(1).map(|m| m.as_str().parse::<u32>().ok())).flatten().collect();
    
    if red.iter().max().unwrap() > &12 || green.iter().max().unwrap() > &13 || blue.iter().max().unwrap() > &14 {
        return false
    }
    return true
}
...
fn part_one() {
    ...
    for line in reader.lines() {
        let s1 = line.unwrap();
        let game: u32 = game_regex.captures(&s1).unwrap().get(1).map(|x| x.as_str().parse::<u32>()).unwrap().expect("Failed to parse game number");
